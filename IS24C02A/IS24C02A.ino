...
void Clear_Memory()
{
  ...
  for (int i = 0; i <= 255; i++)
  {
    writeLocation(EEPROM, (uint8_t) i, 0x00);
  }
...
void Write_Memory()
{
  ...
  for (int i = 0; i <= 255; i++)
  {
    writeLocation(EEPROM, (uint8_t) i, i);
  }
...
void Read_Memory()
{
  ...
  for (int index = 0; index <= 255; index++)
  {
    ...
    uint8_t Temp = readLocation(EEPROM, (uint8_t) index);
    bstr[bcnt++] = Temp;
    Serial.print(" ");
...
void writeLocation(uint8_t device_address, uint8_t reg, uint8_t value)
{
  delayamt = 400;
  Start();
  delayamt = 50;
